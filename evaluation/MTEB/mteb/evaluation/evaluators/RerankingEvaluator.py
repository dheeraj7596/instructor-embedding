...
class RerankingEvaluator(Evaluator):
    ...
    def compute_metrics_batched(self, model):
        ...
        if isinstance(self.samples[0]["query"], str):
            ...
            if self.args.prompt:
                ...
                for s in self.samples:
                    new_sentences.append([instruction, s["query"], 0])
            else:
                new_sentences = [sample["query"] for sample in self.samples]
            ...
            if self.args.prompt:
                ...
                if isinstance(DEFINITIONS[self.args.prompt][self.args.task_name], str):
                    ...
                    instruction = DEFINITIONS[self.args.prompt][self.args.task_name]['query']
                for s in all_query_flattened:
                    new_sentences.append([instruction, s, 0])
            else:
                new_sentences = all_query_flattened
            all_query_embs = model.encode(new_sentences, convert_to_tensor=True, batch_size=self.batch_size)
        else:
        ...
        new_docs = []
        if self.args.prompt and isinstance(DEFINITIONS[self.args.prompt][self.args.task_name], dict):
            instruction = DEFINITIONS[self.args.prompt][self.args.task_name]['corpus']
            for s in all_docs:
                new_docs.append([instruction, s, 0])
            all_docs = new_docs
